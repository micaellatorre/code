...
async function main() {
  ...
  assert.equal((await prisma.product.findUniqueOrThrow({ where: { id: phone.id } })).stock, 0)
  ...
  assert.equal((await prisma.product.findUniqueOrThrow({ where: { id: accessory.id } })).stockAvailable, 4)
  assert.equal(await prisma.cashMovement.count({ where: { tenantId: tenant.id, category: "REVERSAL" } }), 1)
