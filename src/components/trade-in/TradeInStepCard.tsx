...
export default function TradeInStepCard({ stepNumber, title, description, status, isActive, summary, onClick, children }: TradeInStepCardProps) {
  return (
    <section className={`rounded-lg border bg-base-100 ${isActive ? "border-primary/40 shadow-sm" : "border-base-300"}`}>
      <button type="button" className="flex w-full items-start gap-3 p-4 text-left" onClick={onClick}>
        <span className={`mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full text-sm font-bold ${status === "completed" ? "bg-success text-success-content" : status === "active" ? "bg-primary text-primary-content" : "bg-base-300 text-base-content/60"}`}>
