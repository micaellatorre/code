...
export async function POST(
  ...
  try {
    ...
    const { id: _, createdAt: __, updatedAt: ___, stock, stockAvailable, ...dataToCopy } = productToCopy

    const newProduct = await prisma.product.create({
      data: { ...dataToCopy, modelName: `${productToCopy.modelName} Copia #${nextCopyNumber}`, stock: 0, stockAvailable: 0 },
    })
